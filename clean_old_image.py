...
def get_created_date_for_tag(tag, repository, auth, args, cert):
    headers = {'Accept': 'application/vnd.docker.distribution.manifest.v2+json'}

    response = requests.get(args.registry_url + "/v2/" + repository + "/manifests/" + tag,
                            auth=auth, verify=args.no_check_certificate, cert=cert, headers=headers)
...
def main():
    ...
    if args.cert and args.key:
        ...
        CERT = ('/etc/docker/certs.d/docker.kedacom.com:15000/client.cert', '/etc/docker/certs.d/docker.kedacom.com:15000/client.key')
    ...
    response = requests.get(args.registry_url + "/v2/_catalog",
                            auth=auth, verify=args.no_check_certificate, cert=CERT)
    ...
    for repository in repositories:
        ...
        if tags is not None:
            ...
            if args.order == 'name':
                ...
                order_fn = lambda s: get_created_date_for_tag(s, repository, auth, args, CERT)
